...
public sealed class ProductSpecType : ObjectType<ProductSpec>
{
    protected override void Configure(IObjectTypeDescriptor<ProductSpec> d)
    {
        d.Description("A product specification key-value pair.");
        d.Field(x => x.Key).Description("Attribute name (e.g. 'color', 'weight').");
        d.Field(x => x.Value).Description("Attribute value (e.g. 'red', '500g').");
    }
}
...
public sealed class ProductType : ObjectType<ProductDto>
{
    protected override void Configure(IObjectTypeDescriptor<ProductDto> d)
    {
        ...
        d.Field(x => x.IsLowStock)     .Description("True when available stock is 5 or less.");
        d.Field(x => x.Status)         .Description("Draft | Active | Inactive | OutOfStock | Archived");
        d.Field(x => x.Tags)           .Description("Normalised lowercase tags.");
...
public sealed class ProductSummaryType : ObjectType<ProductSummaryDto>
{
    ...
    {
        d.Name("ProductSummary");
        d.Description("Lightweight product summary for lists and search results.");
    }
}

public sealed class PagedProductSummaryResultType : ObjectType<PagedResult<ProductSummaryDto>>
{
    protected override void Configure(IObjectTypeDescriptor<PagedResult<ProductSummaryDto>> d)
    {
        // Explicit name required — HC can't auto-name generic types
        d.Name("PagedProductSummaryResult");
...
/// <summary>Input key-value pair for product attributes.</summary>
